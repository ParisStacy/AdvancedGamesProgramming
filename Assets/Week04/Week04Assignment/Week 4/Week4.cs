...
public class Week4 : MonoBehaviour
{
    ...
    /*
     * Define two functions - one that finds the smallest prime factor of a number (SmallestPrimeFactor()), 
     * and one that returns the number of digits (NumberOfDigits()).  Then, write an changing function (ChangingFunction) that returns
    ...
    {
        bool _changeOutput = false;

        if (_changeOutput) return NumberOfDigits(input);
        else return SmallestPrimeFactor(input);
    }
